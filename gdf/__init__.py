...
class GDF():
    ...
    def sample(self, model, model_inputs, shape, unconditional_inputs=None, sampler=None, t_start=1.0, t_end=0.0, timesteps=20, x_init=None, cfg=3.0, cfg_rho=0.7, sampler_params={}, shift=1, device="cpu"):
        ...
        if cfg is not None:
            ...
            model_inputs = {k:torch.cat([v, v_u], dim=0) if isinstance(v, torch.Tensor) else [torch.cat([vi, vi_u], dim=0) for vi, vi_u in zip(v, v_u)] if isinstance(v, list) else None for (k, v), (k_u, v_u) in zip(model_inputs.items(), unconditional_inputs.items())}
        for i in range(0, timesteps):
            noise_cond = self.noise_cond(logSNR_range[i])
            if cfg is not None:
                pred, pred_unconditional = model(x.repeat(2, 1, 1, 1), noise_cond.repeat(2), **model_inputs).chunk(2)
